...
<head>
<meta charset="utf-8">
<title>Registrar relación</title>
...
<div class="form"> 
<h3>Registrar relacion</h3><br>
<form action="funcion_agregar_tramos_comunidades.php" method="post" id="contactFrm" name="contactFrm">
<div align="left">Seleccione tramo:</div>
<select type="text" class="form-control" name="idtramo" required>
	<?php
		foreach ($ListaTramos as $row) 
		{			
			echo '<option value="'.$row['idTramo'].'">'.$row['NombreTramo'].'</option>';
		}
	?>	
</select><br><br>
<div align="left">Seleccione la(s) comunidad(es) por la que pasa el tramo <br>
(ctrl + click para seleccionar cada una):</div>
<select type="text" class="form-control" name="idcomunidad[]" required multiple>
	<?php
...
<input type="submit" value="Registrar relación" name="submit" class="txt2">
</form>
</div>
</div>
</div>
