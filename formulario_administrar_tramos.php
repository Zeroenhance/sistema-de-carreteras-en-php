...
$tramos = new tramos();
$tablaTramos = $tramos->listar_tramos(); 
?> 
...
<li><a href="formulario_alta_tramo.php"><h3>Ingresar nuevo tramo</h3></a></li>
</ul>
...
<?php
foreach ($tablaTramos as $row) {
echo '<tr>';
echo '<td>' . $row['NombreTramo'] . '</td>';// debemos poner nombre de los campos de la BD            
echo '<td>' . $row['Inicio'] . '</td>';
echo '<td>' . $row['Fin'] . '</td>';
echo '<td>' . $row['NombreCarretera'] . '</td>';
echo '<td>
<a href="formulario_editar_tramo.php?clave='. $row['idTramo'] .'"><img src="img/edit.png" width="50" 
style="cursor:pointer"/></a></td>'; 
echo '<td>
<a href="javascript:seguro(\'funcion_borrar_tramo.php?id='.$row['idTramo'].'\', \''. $row['NombreTramo'] .'\');">
